 ...
 */
namespace Oradt\ServiceBundle\Services\AliPay\lib;
...
class AlipayNotify {
	...
	function verifyNotify($post){
		if(empty($post)) {//判断POST来的数组是否为空
			...
			//生成签名结果
			$isSign = $this->getSignVeryfy($post, $post["sign"],$post["sign_type"]);
	...
	function getSignVeryfy($para_temp, $sign,$sign_type) {
		...
		switch (strtoupper($sign_type)) {
			case "MD5" :
				...
				break;
    		case "RSA" :
        		$isSgin = rsaVerify($prestr, $sign, $this->alipay_config['key']);
        		break;
        	default :
				$isSgin = false;
		}

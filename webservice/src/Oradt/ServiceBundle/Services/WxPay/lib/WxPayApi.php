...
class WxPayApi
{
	 ...
	 * @param int $timeOut
     * @param int $orderType  1:默认【设计师平台订单】 2：橙脉订单
	 * @throws WxPayException
	 * @return 成功时返回，其他抛异常
	 */
	public static function unifiedOrder($inputObj,$orderType = 1, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
            if($orderType == 1){    //设计师平台订单
                $inputObj->SetNotify_url(WxPayConfig::NOTIFY_URL);//异步通知url
            }else{  //橙脉订单
                $inputObj->SetNotify_url(WxPayConfig::WXAPP_IMORA_NOTIFY_URL);//异步通知url
            }
		}
